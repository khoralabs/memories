...
- **`Metadata`** — freeform memory/edge **`properties`** (`nodes.properties` / edge JSON blob). Empty when none. `<meta name="memory:property:…">` is emitted only for allowlisted token-like keys (`[A-Za-z0-9][A-Za-z0-9._:-]{0,127}`, max 128 chars); hostile keys still appear in the visible list (React-escaped).
