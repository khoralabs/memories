...
import type { Memory, SearchHit } from "../core/index";
import {
...
describe("computeLexicalLinkMergeSlice", () => {
  ...
  test("dedupes same neighbor key keeping higher score", () => {
    ...
    if (edge === undefined) {
      throw new Error("expected merged lexical link edge");
    }
    expect((edge.label.props as { similarityScore: number }).similarityScore).toBe(0.9);
  });
