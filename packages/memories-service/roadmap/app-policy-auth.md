...
The HTTP adapter calls into host-provided policy hooks before invoking `MemoriesDatabaseService`. Host app route-level checks move behind a reusable strategy interface rather than living ad hoc in route handlers.
...
The factory receives callbacks or a policy module from the host at startup. The auth package defines the contract; the embedding app supplies the implementation.
...
- Host-specific ontology and namespace builders
...
- `app-policy` for a monolithic host app, or
...
- Tests: mock policy module in `memories-service-auth`; integration tests in embedding apps after migration
