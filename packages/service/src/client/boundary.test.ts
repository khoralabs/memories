...
describe("client/http client/server boundary", () => {
  test("./client/http value graph excludes ontology, remote, and node:crypto", () => {
    const entry = path.join(clientDir, "http.ts");
    const files = reachableFiles(entry);
    const rel = [...files].map((f) => path.relative(packageRoot, f));
    expect(rel.some((f) => f.includes("ontology-client"))).toBe(false);
    expect(rel.some((f) => f.includes("remote-client"))).toBe(false);
