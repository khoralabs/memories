...
const PACKAGE_SUBPATHS: Record<string, string> = {
  "@khoralabs/memories-service": path.join(srcRoot, "index.ts"),
  "@khoralabs/memories-service/http/contracts": path.join(srcRoot, "http/contracts/index.ts"),
  "@khoralabs/memories-service/client/http": path.join(srcRoot, "client/http.ts"),
};

function resolveImport(fromFile: string, spec: string): string | null {
  if (spec === "@khoralabs/memories-service") {
    return PACKAGE_SUBPATHS["@khoralabs/memories-service"] ?? null;
  }
  if (spec.startsWith("@khoralabs/memories-service/")) {
    const mapped = PACKAGE_SUBPATHS[spec];
    if (mapped === undefined) {
      // Unmapped subpaths (e.g. ./storage/sqlite) are out of the root graph by design.
      return null;
    }
    return mapped;
  }
...
describe("service root opt-in sqlite boundary", () => {
  test("root value graph excludes bun:sqlite and storage/sqlite modules", () => {
    const entry = path.join(srcRoot, "index.ts");
    const files = reachableFiles(entry);
    const rel = [...files].map((f) => path.relative(packageRoot, f));
    expect(rel.some((f) => f.includes(`${path.sep}storage${path.sep}sqlite${path.sep}`))).toBe(
      false,
    );
