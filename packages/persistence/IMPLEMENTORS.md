...
This document describes the **operational contract** for [`MemoriesPersistence`](../node/src/persistence/core/persistence/types.ts). Method names and storage types live in `@khoralabs/memories-node`; behavior and ordering are specified here.

The reference SQLite implementation is [`./sqlite/src/persistence.ts`](./sqlite/src/persistence.ts). The wire model is also described in [`packages/spec`](../spec/model/persistence.smithy) (Smithy).

## Relational row shapes (Zod)

Canonical **table Zod schemas**, the composed document schema (`memoriesPersistenceDocumentSchema`), **row TypeScript types** (via `MemoriesPersistenceSchema`), helpers (`zId`, `defineSchema`, `documentValidator`), and **vector payload** rules (`zVectorPayload`, 512–3072 floats) live in **`@khoralabs/memories-node/persistence`**. The reference SQLite strategy defines on-disk tables in [`./sqlite/src/schema.ts`](./sqlite/src/schema.ts) and applies them via [`./sqlite/src/migrations/`](./sqlite/src/migrations/); insert-time `documentValidator` checks use the persistence core package for type alignment. TypeScript backends should use the same `@khoralabs/memories-node/persistence` module so storage rows and merge-time validation stay aligned with `mergeMemory` / `MemoriesPersistence`.
...
| ---------------- | ---- | -------------------- | --------------------------------------------- |
| `MemoriesPersistenceCore` | Lexical mutation, catalog, edges, search-meta text path, lexical search, hydrate | [`MemoriesMutation`](../node/src/persistence/core/persistence/types.ts) (minus vector + optional label-props sync) + lexical half of [`MemoriesRetrieval`](../node/src/persistence/core/persistence/types.ts) | Baseline for any storage implementation. |
...
| `MemoriesPersistenceNeighbors` | Neighbor listing for search | [`MemoriesNeighborIndex`](../node/src/persistence/core/persistence/types.ts) | Omit when `neighborIndex` is `false`. |
...
| `MemoriesPersistenceReads` | Prefetch / export reads | [`MemoriesPersistenceReads`](../node/src/persistence/core/persistence/types.ts) except `listVectorEmbeddingIndexDimensions` (that method is grouped under **Vector** in Smithy) | Thin stores may skip; most backends implement with Core. |
| *(graph topology)* | Namespace edge lists, node labels/properties, incident edges; per-memory labels/properties; load edge by id; load full graph node; node/edge label writes | [`MemoriesGraph`](../node/src/persistence/core/persistence/types.ts) = [`MemoriesGraphIndex`](../node/src/persistence/core/persistence/types.ts) + [`MemoriesGraphMutation`](../node/src/persistence/core/persistence/types.ts) on [`MemoriesPersistence`](../node/src/persistence/core/persistence/types.ts) | Reads return empty when `graphIndex` is `false`. Per-entity reads: `loadNodeLabelsForMemory`, `loadNodePropertiesForMemory`, `loadGraphEdge`, [`loadGraphNode`](../node/src/persistence/core/persistence/types.ts). |
...
- **`memory_provenance`:** Append-only linear chain over **merge** and **delete**. Each row stores `parent_root_hex`, `root_hex`, `event_type`, canonical **`event_json`**, and optional `intent_snapshot_id`. The head is the latest `root_hex` by `_ts_created` (then `_id`). **Genesis parent** for the first link is the fixed 32-byte zero pattern (`00…00` hex), not SQL `NULL`.
- **Leaf + link:** Implemented in `@khoralabs/memories-node/provenance`: event leaf `SHA-256(MEMORIES_EVENT_LEAF_v1 || NUL || UTF-8(canonical_json(event)))`; chain link `SHA-256(parent_32 || leaf_32)` with parent decoded from lowercase hex (or zero bytes at genesis).
...
- Public `SearchParams.options.vectorSearchMethod` may be `"knn"`, `"ann"`, or omitted. Core resolves via [`resolveVectorSearchMethod`](../node/src/persistence/core/persistence/types.ts): explicit selection noops when the capability is off; omitted prefers **ANN then KNN**. `SearchOutput.vectorSearchMethod` reports the method that ran.
...
[`resolveMemoriesBackendCapabilities`](../node/src/persistence/core/persistence/types.ts) merges with [`DEFAULT_MEMORIES_BACKEND_CAPABILITIES`](../node/src/persistence/core/persistence/types.ts) (lexical, vector, neighbor, and multi-namespace search on; **unscoped** off; **`vectorKnnSearch` / `vectorAnnSearch` off** — backends must opt in). Set flags to declare MVP backends:
...
- **Human-readable text:** Use [`formatLabelPropsForSearch`](../core/src/models/label-props-search-text.ts) with an optional per-app [`LabelPropsSearchFormatter`](../core/src/models/label-props-search-text.ts). Reference SQLite passes an optional formatter from [`createMemoriesPersistence`](./sqlite/src/persistence.ts) options.
- **Vectors:** Not indexed on these chunks in v1 (optional follow-up).

## Graph projections (optional)

**`@khoralabs/memories-projections-sqlite`** provides the SQLite projection strategy for UMAP layout, mean-pooled embeddings, and UI preview helpers. Persistence-agnostic callers should depend on `@khoralabs/memories-projections` for layout/source types and choose a strategy package only at the storage boundary.
